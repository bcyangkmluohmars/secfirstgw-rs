        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-navy-800/50">
              {columns.map((col) => (
                <th key={col.key} className="text-left px-4 py-3 text-[11px] text-navy-400 uppercase tracking-wider font-medium">
                  {col.header}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {data.map((row) => (
              <tr key={String(row[keyField])} className="border-b border-navy-800/30 hover:bg-navy-800/20 transition-colors">
                {columns.map((col) => (
                  <td key={col.key} className={`px-4 py-3 ${col.className ?? 'text-gray-300 text-sm'}`}>
            ...
            {data.length === 0 && (
              <tr>
                <td colSpan={columns.length} className="px-4 py-12 text-center text-navy-500">
                  {emptyMessage}
                </td>
              </tr>
            )}
