...
const TLS_DIR: &str = "/data/sfnas-tls";
...
/// The `tls_dir` defaults to [`TLS_DIR`] but can be overridden via `SFNAS_TLS_DIR`.
pub fn load_or_create_tls_config() -> Result<ServerConfig> {
    let tls_dir =
        PathBuf::from(std::env::var("SFNAS_TLS_DIR").unwrap_or_else(|_| TLS_DIR.to_string()));
...
fn generate_and_store(
    ...
    params.distinguished_name.push(
        ...
        rcgen::DnValue::Utf8String("Security First NAS".to_string()),
    );
    params.distinguished_name.push(
        rcgen::DnType::OrganizationName,
        rcgen::DnValue::Utf8String("secfirstnas".to_string()),
    );
