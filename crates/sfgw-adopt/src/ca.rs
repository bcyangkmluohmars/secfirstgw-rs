...
//!
//! Manages the gateway's ML-DSA-65 (FIPS 204) CA keypair used to sign device
...
pub struct CertPayload {
    ...
    /// ML-DSA-65 signature over the above fields (base64).
...
impl GatewayCA {
    ...
    pub async fn init(db: &Db) -> Result<Self> {
        ...
        if let (Some(key_b64), Some(vk_b64), Some(cert_pem)) =
            (existing_key, existing_vk, existing_cert)
        {
            let sk_bytes = B64
                .decode(&key_b64)
                .context("corrupt CA signing key in database")?;
            let vk_bytes = B64
                .decode(&vk_b64)
                .context("corrupt CA verifying key in database")?;
            // Validate that the key material deserialises correctly.
        ...
        // ---- First-boot: generate a new ML-DSA-65 CA keypair ----
    ...
    /// Return the CA public key bytes (ML-DSA-65).
    ...
    pub fn sign(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        .map_err(|e| anyhow::anyhow!("failed to deserialise CA signing key: {e:?}"))?;
        let sig = sk
            .try_sign(data, b"")
    ...
    fn sign_payload_static(signing_key: &[u8], p: &CertPayload) -> Result<String> {
        ...
        .map_err(|e| anyhow::anyhow!("failed to deserialise signing key: {e:?}"))?;
        let sig = sk
